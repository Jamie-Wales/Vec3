...
and generateRecursiveExpr (fname: string) (expr: Expr) : string =
    match expr with
    | ETail(ECall(EIdentifier(id, _), args, _), _)
    | ECall(EIdentifier(id, _), args, _) when 
        match id.Lexeme with 
        | Identifier name -> name = fname
        | _ -> false
        ->
        $"""{{
            {args |> List.mapi (fun i arg -> 
                $"args[{i + 1}] = {generateExpr arg};") |> String.concat "\n            "}
            continue;
        }}"""
    | ECall(callee, args, _) ->
        // Regular function call - evaluate and break
        let argExprs = args |> List.map generateExpr |> String.concat ", "
        match callee with
        | EIdentifier(id, _) ->
            match id.Lexeme with
            | Identifier name ->
                let func = $"vec3_env_get(env, \"{name}\")"
                $"{{ result = vec3_call_function({func}, (Vec3Value*[]){{{func}, {argExprs}}}, {List.length args + 1}); break; }}"
            | _ -> $"{{ result = {generateExpr expr}; break; }}"
        | _ -> $"{{ result = {generateExpr expr}; break; }}"
    | EIf(cond, thenExpr, elseExpr, _) ->
        $"""if (vec3_is_truthy({generateExpr cond})) {{
            {generateRecursiveExpr fname thenExpr}
        }} else {{
            {generateRecursiveExpr fname elseExpr}
        }}"""
    | _ -> 
        $"{{ result = {generateExpr expr}; break; }}"
                ...
                let funcImpl = $"""
                    Vec3Value* {funcName}(Vec3Value** args) {{
                        ...
                        while (1) {{
                            ...
                            // Execute body - either continues loop with new args or breaks with result
                            {generateRecursiveExpr fname body}
