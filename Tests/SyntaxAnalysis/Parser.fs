...
type ParserTests() =
    ...
    member _.Should_ParseTuple() =
        ...
        | Ok(_, statements) ->
            ...
            match statements.Head with
            | SExpression (ETuple ([ ELiteral (LNumber (LInteger 1), _);
                                     ELiteral (LNumber (LInteger 2), _) ],
                                   _),
                          _) -> Assert.Pass()
    ...
    member _.Should_ParseList() =
        ...
        | Ok(_, statements) ->
            ...
            match statements.Head with
            | SExpression (EList ([ ELiteral (LNumber (LInteger 1), _);
                                    ELiteral (LNumber (LInteger 2), _) ],
                                  _),
                          _) -> Assert.Pass()
    ...
    member _.Should_ParseRange() =
        ...
        | Ok(_, statements) ->
            ...
            match statements.Head with
            | SExpression (ERange (ELiteral (LNumber (LInteger 1), _),
                                   ELiteral (LNumber (LInteger 2), _),
                                   _),
                          _) -> Assert.Pass()
    ...
    member _.Should_ParseIndex() =
        ...
        | Ok(_, statements) ->
            ...
            match statements.Head with
            | SExpression (EIndex (EIdentifier ({ Lexeme = Identifier "foo" }, _),
                                   ELiteral (LNumber (LInteger 1), _),
                                   _),
                          _) -> Assert.Pass()
