...
let builtins =
    [ Identifier "plot",
      ...
      Identifier "print",
      VBuiltin((fun args ->
                VOutput $"""{String.concat " " (List.map valueToString args)}"""), "Print")
      Identifier "BUILTIN_SQRT",
      VBuiltin((fun args ->
          match args with
          | [ VNumber(VFloat f) ] ->
              VNumber(VFloat(sqrt f))
          | [ VNumber(VInteger i) ] ->
              VNumber(VFloat(sqrt(float i)))
      ...
      Identifier "error",
        VBuiltin((fun args ->
            ...
            | _ -> raise <| InvalidProgramException "Expected a string for error"), "Error")
